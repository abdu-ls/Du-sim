...
class DuelingDQNAgent:
    ...
    def compute_reward(self, outcome, task, alpha=0.6, beta=0.4, lam=50.0):
    	...
    	"""
    	priority_weight = 4 - task["priority"]   # 3=critical, 2=urgent, 1=routine

    	# Normalise time: scale by the task deadline so penalty is always 0 to ~2
    	normalised_time   = outcome["time"] / task["deadline"]

    	# Normalise energy: divide by a typical max energy value (0.5 J)
    	normalised_energy = outcome["energy"] / 0.5

    	latency_penalty   = alpha * priority_weight * normalised_time
    	energy_penalty    = beta  * normalised_energy
