...
class DuelingDQNAgent:
    ...
    def compute_reward(self, outcome, task, alpha=0.6, beta=0.4, lam=50.0):
        """
        Reward function from Equation (10) in the paper.
        Priority P_i multiplies the latency penalty so the agent
        is punished more for delaying critical tasks.

        Note: lower P_i = higher urgency (1=critical). We invert
        to make the multiplier larger for critical tasks: weight = 4 - P_i.
        """
        priority_weight = 4 - task["priority"]   # 3 for critical, 2 urgent, 1 routine
        latency_penalty = alpha * priority_weight * outcome["time"]
        energy_penalty  = beta  * outcome["energy"]
