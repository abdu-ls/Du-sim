...
class IoMTDevice:
    ...
    def generate_task(self):
        """
        Creates a single IoMT task from a random vital sign reading.
        Returns the task tuple and the priority label P_i.
        """
        vital = np.random.choice(VITAL_SIGNS)
        value = generate_sensor_reading(vital)
        priority = classify_priority(vital, value)

        # Task data size in bits (0.1 MB to 5 MB)
        data_size = np.random.uniform(0.1e6 * 8, 5e6 * 8)
        # Computation demand in CPU cycles (10^7 to 10^9)
        cpu_cycles = np.random.uniform(1e7, 1e9)
        # Maximum deadline in seconds (shorter for critical tasks)
        if priority == 1:
            deadline = 0.050   # 50 ms for critical
        elif priority == 2:
            deadline = 0.100   # 100 ms for urgent
        else:
            deadline = 5.0     # 5 seconds for routine
...
class MECServer:
    ...
    def __init__(self, server_id):
        ...
        self.load = np.random.uniform(0.1, 0.4)
...
class SDNController:
    ...
    def compute_outcome(self, task, action):
        ...
        for server in self.mec_servers:
            server.load = np.clip(server.load + np.random.uniform(-0.05, 0.05), 0.05, 0.95)
