...
def smooth(series, window=15):
    """Rolling average to reduce noise and reveal the underlying trend."""
...
# Dueling DQN — smoothed mean with raw std band
ax.plot(smooth(conv_d["mean"]), color="#1f77b4",
        label="Dueling DQN", linewidth=2.0)
ax.fill_between(conv_d.index,
                conv_d["mean"] - conv_d["std"],
                conv_d["mean"] + conv_d["std"],
                alpha=0.15, color="#1f77b4")

# Standard DQN — smoothed mean with raw std band
ax.plot(smooth(conv_s["mean"]), color="#ff7f0e",
        label="Standard DQN", linestyle="--", linewidth=2.0)
...
ax.set_ylabel("Cumulative Reward (smoothed, window=15)")
