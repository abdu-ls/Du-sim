...
conv = pd.read_csv("convergence_data.csv", index_col="episode")
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(conv["mean"], color="#1f77b4", label="Dueling DQN", linewidth=1.5)
ax.fill_between(conv.index,
                conv["mean"] - conv["std"],
                conv["mean"] + conv["std"],
                alpha=0.25, color="#1f77b4")
ax.set_xlabel("Training Episode"); ax.set_ylabel("Cumulative Reward")
ax.set_title("Training Convergence"); ax.legend()
plt.tight_layout(); plt.savefig("fig2_convergence.pdf", dpi=300); plt.close()
